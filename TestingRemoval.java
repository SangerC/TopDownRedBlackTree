...
//		System.out.println("End testing");	
	...
	public void testingLogBehaviorOfRemoval(){
		...
		Iterator<RedBlackTree.BinaryNode> it = t.iterator();
		while (it.hasNext()){
			RedBlackTree.BinaryNode n = it.next();
			if (n.getLeftChild() != null && n.getLeftChild().getElement().compareTo(n.getElement()) > 0) fail();
			if (n.getRightChild() != null && n.getRightChild().getElement().compareTo(n.getElement()) < 0) fail();
		}
		
		for (int i = 0; i < nums/2; i++) t.remove(i);
		it = t.iterator();
		while (it.hasNext()){
			RedBlackTree.BinaryNode n = it.next();
			if (n.getLeftChild() != null && n.getLeftChild().getElement().compareTo(n.getElement()) > 0) fail();
		...
		for (int i = nums/2; i < nums; i++) t.remove(i);
